...
var createProjector = require('./CreateProjector.js');
...
var outFileName = path.join(__dirname, '\\..\\graphs', fileName);
...
function saveResults() {
    ...
    //console.log("fuuuuuuuuuuuuuuuuuuck Next");
    //console.log(lonLatBbox.cx);
    ...
    graph.forEachNode(node => {
        ...
        var nodeData = { x: data.lon, y: data.lat };
...
function saveNodes(fileName, graph, nodeIdMap) {
    ...
    var buf = new Buffer.alloc(nodeCount * 4 * 2);
    var idx = 0;
    graph.forEachNode(p => {
...
function saveLinks(fileName, graph, nodeIdMap) {
    var buf = new Buffer.alloc(graph.getLinksCount() * 4 * 2);
    var idx = 0;
    graph.forEachLink(l => {
